...
function createPromise (dep) {
  ...
  if (!window.exampleApp.loader[dep.url]) {
    ...
    const promise = new Promise((resolve, reject) => {
      const handleResponse = (e) => {
        if (e.error || (e.type && e.type === 'error')) {
          ...
          window.exampleApp.loader[dep.url].hasLoaded = true
          resolve()
        }
      }
    ...
    window.exampleApp.loader[dep.url] = { type: dep.is, hasLoaded: false, promise }
    return promise
  } else {
    return window.exampleApp.loader[dep.url].promise
  }
}
...
export function loadExternal (...items) {
  ...
  window.exampleApp.loader = window.exampleApp.loader || {}
