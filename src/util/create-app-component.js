...
class FicusAppShell extends HTMLElement {
  connectedCallback () {
    if (!this.classList.contains(appShellTagName)) {
      ...
      const script = createScript(`/app-ui-${media}/main.js`, { type: 'module' })
...
function getDesktopMediaQuery () {
  return window.ficusShellRuntime && window.ficusShellRuntime.desktopMediaQuery
    ? window.ficusShellRuntime.desktopMediaQuery
    : '(min-width: 1280px)'
}
