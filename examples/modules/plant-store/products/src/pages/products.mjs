  ...
  createCustomElement(
    ...
    ExtensionBuilder
      ...
      .withStore({
        ...
        products: getAppState(storeNames.PRODUCTS),
      })
      .withI18n(getI18n())
      ...
      .create({
        ...
        currency (amount) {
          return this.i18n ? this.i18n.t('products.currency', { amount }) : 'GBP'
        },
        mounted () {
          this.store.layout.setPageTitle(this.pageTitle)
          this.store.products.loadProductsIfNotLoaded()
        },
        render () {
          return html`
            <fas-page page-title="${this.pageTitle}">
              <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
                ${this.store.products.getProducts().map(product => html`
                  <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
                    <button class="text-left" type="button" onclick="${() => this.addItemToBasket(product)}">
                      ...
                      <p class="pt-1 text-gray-900">${this.currency(product.price)}</p>
                    </button>
                  </div>
                `)}
              </div>
